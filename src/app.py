...
@app.route('/highlight_pdf/<int:id>', methods=['POST'])
def highlight_pdf(id):
    ...
    try:
        ...
        # Upload highlighted PDF to Supabase Storage
        result = supabase.storage.from_(BUCKET_NAME).upload(
        ...
        # Update database entry for highlighted PDF
        update_result = supabase.table('Contract').update({
